...
declare -A GEN_CLIB_PRO_LOGO_DATA=(
    [OWNER]="vroncevic"
    [REPO]="${GEN_CLIB_PRO_TOOL}"
    [VERSION]="${GEN_CLIB_PRO_VERSION}"
    [LOGO]="${GEN_CLIB_PRO_LOGO}"
)
...
function __gen_clib_pro {
    local PN=$1
    if [ -n "${PN}" ]; then
        display_logo GEN_CLIB_PRO_LOGO_DATA
        ...
        load_conf "$GEN_CLIB_PRO_CFG" config_gen_clib_pro
        STATUS_CONF=$?
        declare -A config_gen_clib_pro_util=()
        load_util_conf "$GEN_CLIB_PRO_UTIL_CFG" config_gen_clib_pro_util
        STATUS_CONF_UTIL=$?
        ...
        if [ $STATUS -eq $NOT_SUCCESS ]; then
            MSG="Force exit!"
            info_debug_message_end "$MSG" "$FUNC" "$GEN_CLIB_PRO_TOOL"
            exit 129
        fi
        ...
        load_util_conf "${GEN_CLIB_PRO_HOME}/conf/${PROJECT_SET}" project_set
        STATUS=$?
        if [ $STATUS -eq $NOT_SUCCESS ]; then
            MSG="Force exit!"
            info_debug_message_end "$MSG" "$FUNC" "$GEN_CLIB_PRO_TOOL"
        ...
        local CLTF="${GEN_CLIB_PRO_HOME}/conf/${CLT}"
        ...
        logging GEN_CLIB_PRO_LOGGING
        info_debug_message_end "Done" "$FUNC" "$GEN_CLIB_PRO_TOOL"
        TREE=$(which tree)
        check_tool "${TREE}"
        STATUS=$?
